...
from umierrorcorrect.src.check_args import check_args_fastq, get_sample_name
...
def parseArgs():
    parser = argparse.ArgumentParser(
        description=f"UmiErrorCorrect v. {__version__}. Pipeline for analyzing  barcoded amplicon sequencing data with \
    ...
    group2.add_argument(
        ...
        help="Length of UMI sequence (number of nucleotides). The UMI is assumed to \
                             be located at the start of the read. Required",
        required=True,
    )
    group2.add_argument(
        "-sl",
    ...
    group2.add_argument(
        ...
        help="Name of library prep, Either 'single' or 'paired', for single end or paired \
                             end data respectively, [default = %(default)s]",
        default="paired",
    )
    group2.add_argument(
        "-dual",
